...
def _check_maybe(t):
	if isinstance(t, tensor_array_ops.TensorArray):
		raise TypeError(
			"TensorArray state is not supported by PCGNBeamSearchDecoder: %s" % t.name)
	if t.shape.ndims is None:
		raise ValueError(
			"Expected tensor (%s) to have known rank, but ndims == None." % t)


class PCGNBeamSearchDecoder(decoder.Decoder):
	...
	def __init__(self,
				 ...
				 beam_width,
				 output_layer=None,
				 use_external_desc_express=False,
				 embed_desc=None,
				 blog_desc_inetract=None,
				 feat_embed=None,
				 use_external_feat_express=False,
				 user_map=None,
				 length_penalty_weight=0.0):
		"""Initialize the PCGNBeamSearchDecoder.
		...
		# PCGN output layer
		self._use_external_desc_express=use_external_desc_express
		self._embed_desc = embed_desc
		self._feat_embed=feat_embed
		self._blog_desc_inetract = blog_desc_inetract
		self._use_external_feat_express=use_external_feat_express
	...
	def external_personality_express(self, decoder_logits_train,
								   keys,blog_desc_inetract,feat_emb=None,use_external_feat_express=False,user_map=None):  # keys Processed memory, shape `[batch_size, max_time, num_units]`.
		"""
		Args:
		...
		"""
		batch_size, num_units = decoder_logits_train.get_shape()
		t_decoder_logits = decoder_logits_train
		#t_decoder_logits=blog_desc_inetract(t_decoder_logits)
		context = self.get_context(t_decoder_logits, keys, blog_desc_inetract, batch_size, num_units)

		if use_external_feat_express:
			context = tf.concat([context, feat_emb], axis=-1)
		if user_map is not None:
			context = tf.matmul(context, user_map)
		cnt = tf.concat([t_decoder_logits, context], axis=-1)
		return cnt
	...
	def step(self, time, inputs, state, name=None):
		...
		with ops.name_scope(name, "BeamSearchDecoderStep", (time, inputs, state)):
			...
			# ======================================================================
			if self._use_external_desc_express:
				if self._use_external_feat_express:
					_feat_embed=self._feat_embed
				else:
					_feat_embed=None
				align_decoder_logits = self.external_personality_express(cell_outputs, self._embed_desc,
															   self._blog_desc_inetract,_feat_embed,use_external_feat_express=self._use_external_feat_express,user_map=self._user_map)
				gen_log_probs = tf.nn.log_softmax(self._output_layer(align_decoder_logits))  # 得到普通词的概率分布
			else:
				gen_log_probs = tf.nn.log_softmax(self._output_layer(cell_outputs))
			raw_log_probs = gen_log_probs
